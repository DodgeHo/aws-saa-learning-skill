**学习目标**
- 理解 Amazon ECS 的启动类型：EC2 与 Fargate
- 掌握任务角色与实例配置文件的区别与用途
- 了解负载均衡与持久化（EFS）在 ECS 中的集成方式

**重点速览**
- EC2 启动类型需自建/维护 EC2 实例并运行 ECS 代理
- Fargate 为无服务器运行模式，免管理 EC2 主机
- 任务角色用于授予容器访问 AWS 服务的权限；实例配置文件用于 EC2 代理
- ALB 常用于对外暴露任务，EFS 可用于任务间共享持久存储

**详细内容**
ECS 有两种主要启动类型：
- EC2：你要管理底层 EC2 实例，安装 ECS 代理，代理将实例注册到指定集群；镜像拉取与容器运行由这些实例提供计算资源。
- Fargate：无需管理实例，按任务定义的 CPU/内存直接运行容器，便于按需扩缩；考试常建议优先使用 Fargate。

权限方面区分两类角色：实例配置文件（用于 EC2 实例，供代理与实例调用 API）和 ECS 任务角色（授予容器自身的权限，例如访问 S3、DynamoDB、参数存储等）。在任务定义中指定任务角色。

负载均衡：应用程序负载均衡器（ALB）与 ECS 集成良好，可将请求路由到任务；网络负载均衡器（NLB）用于高吞吐/低延迟场景。持久化存储方面，可使用 Amazon EFS 将文件系统挂载到任务上（EFS 与 Fargate 兼容），S3 不能直接作为挂载文件系统。

**自测问题**
- EC2 与 Fargate 启动类型的主要区别是什么？
- 任务角色和实例配置文件各自用于什么场景？
- 如何给多个任务共享持久化文件存储？
---
