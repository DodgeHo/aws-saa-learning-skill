...
- 实操配置 S3 桶的 CORS 设置并验证跨源资源能否被静态网站加载。
- 能通过修改 `index.html` 和上传资源来触发跨源请求并观察浏览器控制台的错误或成功头。

## 重点速览

- 将演示页面拆分成主站与“其他来源”两个桶：主站请求另一个桶的资源会触发 CORS。
- 若目标桶未配置 CORS，浏览器会在控制台报错（缺少 Access-Control-Allow-* 头），添加正确的 CORS JSON 后请求将被允许。

## 详细内容（实操步骤概览）

1. 准备主站与资源
- 编辑 `index.html`（启用示例代码块），并上传 `index.html` 与 `extra-page.html` 到同一桶以验证本源请求能正常工作。

2. 创建另一个桶作为“其他来源”
- 新建桶（例如 `demo-other-origin-*`），在 Properties 中启用静态网站托管，并通过桶策略或权限将其设为公开（仅为教学用途）。

3. 上传资源并验证初次请求
- 将 `extra-page.html` 上传到“其他来源”桶，尝试从主站通过完整 URL 请求该页面，若未配置 CORS，浏览器会显示跨源阻止错误（缺少 `Access-Control-Allow-*` 头）。

4. 为目标桶添加 CORS 配置
- 在目标桶的 `Permissions -> CORS configuration` 中粘贴合适的 JSON，指定 `AllowedOrigins` 为主站的静态网站 URL（去掉末尾斜杠），并设置允许的方法（如 GET）和头。

5. 验证
- 刷新主站页面，观察资源成功加载；在浏览器 Network 面板查看响应头，确认出现 `Access-Control-Allow-Origin: <主站>` 和 `Access-Control-Allow-Methods: GET` 等。

## 自测问题

1. 若目标桶未设置 CORS，浏览器会如何提示？在哪可以看到这些信息？
2. 在 S3 的 CORS JSON 中，如何指定只允许某个 origin？如何允许所有 origin？
3. 为什么要在浏览器的 Network 面板查看响应头来验证 CORS 配置？
