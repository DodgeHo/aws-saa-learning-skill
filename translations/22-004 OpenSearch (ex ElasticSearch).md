...
- 了解 Amazon OpenSearch 的用途、核心组件与与 Elasticsearch 的关系。 
- 掌握索引、分片、节点类型与常见查询/聚合场景。 

## 重点速览

- OpenSearch（基于 ElasticSearch）用于实时日志分析、搜索与可视化；适合时序数据与全文检索。 
- 架构包含主节点、数据节点与协调节点；数据通过索引分片分布以实现水平扩展。 

## 详细内容

- 用例与特性：
	- 日志和指标分析、应用搜索、仪表盘与告警；与 Kibana 类似的 OpenSearch Dashboards 提供可视化。 
	- 支持全文检索、布尔查询、聚合与近实时查询。 

- 部署与扩展：
	- 集群由主节点（管理元数据）、数据节点（存储与查询）与协调节点（路由请求）组成；通过增加分片与节点来扩展吞吐与容量。 
	- 注意索引模板、映射设计与分片策略，会直接影响查询性能与磁盘利用。 

- 安全与运维：
	- 访问控制、TLS、索引生命周期管理（ILM）、快照备份至 S3 等是典型运维任务。 

## 自测问题

- OpenSearch 的典型应用场景有哪些？
- 描述主节点与数据节点的职责差异。 
- 如何通过索引设计优化写入与查询性能？
